...
For each reply (≤150-word position + JSON): if the debater's transcript file does not exist, write the task output to that path yourself. Extract the final ```json block to a temp file, then gate it:
